	...
	* Устанавливаем формулы расчета
		...
		* Источник
			...
			* Артикул
				...
				И в таблице 'TreeOfFilterParametersDB' из выпадающего списка с именем 'ParameterCalculationMethod' я выбираю точное значение "Saved filter setting"
				И в таблице 'TreeOfFilterParametersDB' я активизирую поле с именем 'DefiningMethodClarification'
				И в таблице 'TreeOfFilterParametersDB' из выпадающего списка с именем 'DefiningMethodClarification' я выбираю по строке "Catalog_Product range  Reference<>1С:Pредприятие 8 КОРP. Клиентская лицензия for 100 рабочих мест"
				И в таблице 'TreeOfFilterParametersDB' я завершаю редактирование строки
				Когда открылось окно "Data source (create) *"
				И я нажимаю на кнопку с именем 'FormWriteAndClose'
				И я жду закрытия окна "Data source (create) *" в течение 20 секунд
...
Сценарий: 07.09 Создание экземпляра отчета - "VA - Non-Numbers (recipient)"  
	...
	* Рассчитываем документ и сравниваем итоговый документ
		...
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен макету "3_Budgeting\Templates\07\VA_НечисловыеFunctionsDestination_Mercury.mxl"
...
Сценарий: 07.10 Расчет с удалением показателей
	...
	* Проверяем форму
		...
		И элемент формы с именем 'FormForИмпорта' стал равен "VA - Non-Numbers (recipient)"
	...
	* Рассчитываем документ и сравниваем итоговый документ
		И я нажимаю на кнопку с именем 'FormFillByDefault'
		Тогда табличный документ 'SpreadsheetFieldTemlate' равен макету "3_Budgeting\Templates\07\VA_НечисловыеFunctionsDestination_Venus.mxl"					
	...
	* Проверяем движения
		И движения экземпляра отчета стали равны "3_Budgeting\Templates\07\VA_НечисловыеFunctionsDestination_Venus_Движения.mxl"
...
Сценарий: 07.11 Создание экземпляра отчета - "VA - Calculation in code" 
	...
	* Записываем документ и сверяем движения	
		...
		Дано Табличный документ 'ReportSpreadsheetDocument' равен макету "3_Budgeting\Templates\07\VA_CalculationINКоде_Движения.mxl" по шаблону
...
Сценарий: 07.12 Создание экземпляра отчета - "VA - Grouping by analytics"
	...
	* Проверяем движения	
		...
		Дано Табличный документ 'ReportSpreadsheetDocument' равен макету "3_Budgeting\Templates\07\VA_GroupingПоАналитикам_Движения.mxl" по шаблону
